...
func ConfigureNotificationRoutes(appRoute *echo.Group, server *s.Server) {
	...
	notification := appRoute.Group("/notification", authentication.CheckAuthentication(server))
	...
	fcmToken := appRoute.Group("/fcm-token", authentication.CheckAuthentication(server))
